...
public class ProfilResourceIT {
    ...
    private static final Integer DEFAULT_ETAT_PROFIL = 1;
    private static final Integer UPDATED_ETAT_PROFIL = 2;
    ...
    public void setup() {
        ...
        final ProfilResource profilResource = new ProfilResource(profilRepository,menuRepository,moduleRepository,rubriqueRepository);
    ...
    public static Profil createEntity(EntityManager em) {
        Profil profil = new Profil()
            .etatProfil(DEFAULT_ETAT_PROFIL)
    ...
    public void createProfil() throws Exception {
        ...
        assertThat(testProfil.getEtatProfil()).isEqualTo(DEFAULT_ETAT_PROFIL);
        assertThat(testProfil.getLibelleProfil()).isEqualTo(DEFAULT_LIBELLE_PROFIL);
    }
    ...
    public void getAllProfils() throws Exception {
        ...
        restProfilMockMvc.perform(get("/api/profils?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].etatProfil").value(hasItem(DEFAULT_ETAT_PROFIL)))
            .andExpect(jsonPath("$.[*].libelleProfil").value(hasItem(DEFAULT_LIBELLE_PROFIL.toString())));
    }
    ...
    public void getProfil() throws Exception {
        ...
        restProfilMockMvc.perform(get("/api/profils/{id}", profil.getId()))
            ...
            .andExpect(jsonPath("$.etatProfil").value(DEFAULT_ETAT_PROFIL))
            .andExpect(jsonPath("$.libelleProfil").value(DEFAULT_LIBELLE_PROFIL.toString()));
    }
    ...
    public void updateProfil() throws Exception {
        ...
        // Update the profil
        Profil updatedProfil = profilRepository.findById(profil.getId())
            .orElseThrow(() -> new IllegalStateException("Profil not found in database"));
        // Disconnect from session so that the updates on updatedProfil are not directly saved in db
        em.detach(updatedProfil);
        updatedProfil
            .etatProfil(UPDATED_ETAT_PROFIL)
        ...
        Profil testProfil = profilList.get(profilList.size() - 1);
        assertThat(testProfil.getEtatProfil()).isEqualTo(UPDATED_ETAT_PROFIL);
        assertThat(testProfil.getLibelleProfil()).isEqualTo(UPDATED_LIBELLE_PROFIL);
    }
