...
import {IProfil} from "../../shared/model/profil.model";
...
export class UserMgmtUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.route.data.subscribe(({ user }) => {
            ...
            if(this.user.id ){
                this.userService.getEmploye(this.user.id).subscribe(
                    (data)=>{
                        if(data.body.length!=0){
                            this.employe=data.body[0];
                            this.userService.getAllProfil().subscribe(
                                (data)=>{


                                    for(let a of data.body){
                                        if(a.id==this.employe.profil.id){
                                            a.cocher=true;
                ...
                this.employeService.allUser().subscribe(
                    (data)=>{
        ...
        jQuery(document).ready(setTimeout(function() {
            ...
            $('#dane1').on('change',function () {
                ...
                $.all1= $(this).val();
                comp.dane(comp.users.filter((a)=>a.login==$.all1));
                console.log($.all1);
                console.log("dane");
            })
    ...
    save() {
        this.isSaving = true;
        if (this.user.id !== null && !this.user.x) {

            //this.user.employe.profil=this.profils.filter((a)=>(a.libelleProfil==$.all))[0]
            this.userService.getEmploye(this.user.id).subscribe(
                (data)=>{
                    data.body[0].profil=[];
                    data.body[0].telUtilisateur=this.employe.telUtilisateur;
                        data.body[0].profil=this.profils.filter((a)=>(a.libelleProfil==$.all))[0];
                    this.employeService.update(data.body[0]).subscribe(
                        ()=>{
                            console.log("gueum gueum")
                        }
                    )
                }
            );
            ...
            //this.user.employe.profil=this.profils.filter((a)=>(a.libelle==$.all))[0]
            this.user.authorities=["ROLE_ADMIN","ROLE_USER"];
            this.userService.update(this.user).subscribe(
                ()=> {
                            this.employe.user =this.user;

                           /* for(let x of this.profils){
                                if(this.positionprofil==x.id)
                                    this.employe.profil=x;
                            }*/
                            this.employe.profil=this.profils.filter((a)=>(a.libelleProfil==$.all))[0];
