...
type RestOf<T extends IInscription | NewInscription> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class InscriptionService {
  ...
  addInscriptionToCollectionIfMissing<Type extends Pick<IInscription, 'id'>>(
    ...
    if (inscriptions.length > 0) {
      const inscriptionCollectionIdentifiers = inscriptionCollection.map(inscriptionItem => this.getInscriptionIdentifier(inscriptionItem));
  ...
  protected convertDateFromClient<T extends IInscription | NewInscription | PartialUpdateInscription>(inscription: T): RestOf<T> {
    return {
      ...inscription,
      date: inscription.date?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restInscription: RestInscription): IInscription {
    return {
      ...restInscription,
      date: restInscription.date ? dayjs(restInscription.date) : undefined,
    };
  }
