...
type RestOf<T extends IApplication | NewApplication> = Omit<T, 'dateVente' | 'dateDebut' | 'dateFin'> & {
...
export class ApplicationService {
  ...
  protected convertDateFromClient<T extends IApplication | NewApplication | PartialUpdateApplication>(application: T): RestOf<T> {
    return {
      ...application,
      dateVente: application.dateVente?.format(DATE_FORMAT) ?? null,
      dateDebut: application.dateDebut?.format(DATE_FORMAT) ?? null,
      dateFin: application.dateFin?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restApplication: RestApplication): IApplication {
    return {
      ...restApplication,
      dateVente: restApplication.dateVente ? dayjs(restApplication.dateVente) : undefined,
      dateDebut: restApplication.dateDebut ? dayjs(restApplication.dateDebut) : undefined,
      dateFin: restApplication.dateFin ? dayjs(restApplication.dateFin) : undefined,
    };
  }
