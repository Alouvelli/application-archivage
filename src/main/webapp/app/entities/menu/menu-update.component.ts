...
export class MenuUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.rubriqueService
            ...
            .subscribe((res: IRubrique[]) => {
                this.rubriques = res;
                this.rubriques=this.rubriques.filter((a)=>a.etatRubrique==true);
                for(let x of this.rubriques){

                    if(this.menu.rubrique.id) {
                        if (this.menu.rubrique.id == x.id) {
                            x.cocher = true;
                        }
                    }
    ...
    save() {
        if(!$.all && this.rubriques.filter((a)=>a.cocher==true).length==0){
            this.invalid=false;
        }else {
            this.isSaving = true;
            if (this.menu.id !== undefined) {
                if(!$.all   && this.rubriques.filter((a)=>a.cocher==true).length!=0 && this.active==0){
                    console.log("aaa")
                    this.invalid=false
                    this.isSaving = false;
                }else {
                    ...
                    if ($.all) {
                        ...
                        this.menu.rubrique=this.rubriques.filter((a)=>(a.libelleRubrique==$.all))[0]
    ...
    changeEtat(){
        if(this.menu.etatMenu==1){
            this.menu.etatMenu=0
        }else{
            this.menu.etatMenu=1
        }
