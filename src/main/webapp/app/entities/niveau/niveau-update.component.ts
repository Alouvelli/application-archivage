...
import { TypeDocumentService } from 'app/entities/type-document';
...
export class NiveauUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.typeDocumentService
            .query()
            .pipe(
                filter((mayBeOk: HttpResponse<ITypeDocument[]>) => mayBeOk.ok),
                map((response: HttpResponse<ITypeDocument[]>) => response.body)
            )
            .subscribe((res: ITypeDocument[]) => {this.typedocuments = res;

                for(let x of this.typedocuments){
                    if( this.niveau.typeDocuments.length>0){
                        for(let a of this.niveau.typeDocuments){
                            if(x.id==a.id){
                                x.selectedSite=true;
                            }
                        }}
        ...
        if (this.niveau.id !== undefined) {
            ...
            if($.all) {
                ...
                for (let b of $.all) {
                    ...
                    for (let dane of this.typedocuments) {

                        if (b == dane.libelle) {
                            this.niveau.typeDocuments.push(dane)
                        }
                    }
                };
            }else {
                this.niveau.typeDocuments=this.typedocuments.filter((a)=>a.selectedSite==true);
            }
    ...
    trackTypeDocumentById(index: number, item: ITypeDocument) {
        return item.id;
    }
    ...
    changeEncours(){
        if(this.niveau.encours==1){
            this.niveau.encours=0
        }else{
            this.niveau.encours=1
        }
