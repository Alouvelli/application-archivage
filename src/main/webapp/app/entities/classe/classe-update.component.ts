...
export class ClasseUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.filiereService
            ...
            .subscribe((res: IFiliere[]) => {this.filieres = res
                for(let x of this.filieres){


                    if(this.classe.filiere.id==x.id){
                        x.cocher=true;
                    }
        ...
        this.niveauService
            ...
            .subscribe((res: INiveau[]) => {this.niveaus = res;
                for(let x of this.niveaus){


                    if(this.classe.niveau.id==x.id){
                        x.cocher=true;
                    }
    ...
    save() {
        ...
        if (this.classe.id !== undefined) {
            if($.all){
                this.classe.filiere=this.filieres.filter((a)=>(a.libelle==$.all))[0];
            }else {
                this.classe.filiere=this.filieres.filter((a)=>(a.cocher==true))[0];
            }

            if($.all1){
                this.classe.niveau=this.niveaus.filter((a)=>(a.libelle==$.all1))[0];
            }else {
                this.classe.niveau=this.niveaus.filter((a)=>(a.cocher==true))[0];
            }

            this.subscribeToSaveResponse(this.classeService.update(this.classe));
        } else {
            this.classe.filiere=this.filieres.filter((a)=>(a.libelle==$.all))[0];
            this.classe.niveau=this.niveaus.filter((a)=>(a.libelle==$.all1))[0];
            this.subscribeToSaveResponse(this.classeService.create(this.classe));
        }
    }

    protected subscribeToSaveResponse(result: Observable<HttpResponse<IClasse>>) {
        result.subscribe((res: HttpResponse<IClasse>) => this.onSaveSuccess(), (res: HttpErrorResponse) => this.onSaveError());
    }
