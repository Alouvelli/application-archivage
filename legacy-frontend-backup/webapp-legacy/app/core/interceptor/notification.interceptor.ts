    ...
    return next.handle(request).pipe(
      tap((event: HttpEvent<any>) => {
        if (event instanceof HttpResponse) {
          ...
          if (alert) {
            // SOLUTION SIMPLE: Utiliser seulement 'message' au lieu de 'translationKey'
            this.alertService.addAlert({
              type: 'success',
              message: alert,
              // Si vous voulez garder les paramètres, vous pouvez les ajouter au message
              // message: alertParams ? `${alert} - ${alertParams}` : alert
});
          }
        }
      }),
