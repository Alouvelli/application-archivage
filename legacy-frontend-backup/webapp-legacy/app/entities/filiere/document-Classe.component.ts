...
@Component({
    selector: 'jhi-document-classe',
    templateUrl: './document-classe.component.html'
})
export class DocumentClasseComponent implements OnInit {
        ...
        this.filiereService.query().subscribe(
            (data)=>{
               ...
               let y=data.body.filter((a)=>a.libelle==x)[0].id;
                console.log(y);
               this.classeService.findclasseById(y).subscribe(
                    (data)=>{
                        console.log(data.body);
                        this.classe=[];
    ...
    save() {
        ...
        if (this.filiere.id !== undefined) {
            this.filiere.departement=this.departements.filter((a)=>(a.libelle==$.all))[0];
