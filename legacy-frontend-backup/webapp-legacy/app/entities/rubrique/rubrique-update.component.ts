...
export class RubriqueUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.moduleService
            ...
            .subscribe((res: IModule[]) => {
                this.modules = res;
                this.modules=  this.modules.filter((a)=>a.etatModule==true);

                for(let x of this.modules){


                    if(this.rubrique.module.id==x.id){
                        x.cocher=true;
                    }
    ...
    changeEtat(){
        if(this.rubrique.etatRubrique==1){
            this.rubrique.etatRubrique=0
        }else{
            this.rubrique.etatRubrique=1
        }
    ...
    save() {
        ...
        if(!$.all && this.modules.filter((a)=>a.cocher==true).length==0){
            this.invalid=false
        }else {
            this.isSaving = true;
            if (this.rubrique.id !== undefined) {
                if(!$.all   && this.modules.filter((a)=>a.cocher==true).length!=0 && this.active==0){
                    ...
                    if ($.all) {
                        ...
                        this.rubrique.module=this.modules.filter((a) => a.libelleModule == $.all)[0];
