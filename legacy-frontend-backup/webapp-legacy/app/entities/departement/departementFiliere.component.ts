...
import {IFiliere} from "../../shared/model/filiere.model";

@Component({
    selector: 'jhi-departementFiliere',
    templateUrl: './departementFiliere.component.html'
})
export class DepartementFiliereComponent implements OnInit, OnChanges{
    departement: IDepartement;
    isSaving: boolean;
    filiere: IFiliere[];

    constructor(protected departementService: DepartementService,
    ...
    dane(){
        let x=this.activatedRoute.snapshot.paramMap.get("name");
        console.log(x);

        this.departementService.query().subscribe(
            (data)=>{
                let y=data.body.filter((a)=>a.libelle==x)[0].id;
                console.log(y);
                this.filiereService.findfiliereById(y).subscribe(
                    (data)=>{
                        this.filiere=[];
                        this.filiere=data.body;
                    }
                )
            }
        );
